...
@Component({
   selector: 'st-radio',
   template: `
     <label [attr.for]="inputId" class="st-radio sth-radio" [ngClass]="{'disabled': disabled}" #label [attr.id]="qaTag">
        <input 
           type="radio" 
           [id]="inputId"
           [attr.id]=""
        ...
        <div class="custom-radio sth-radio-custom" [ngClass]="{'checked' : checked}"></div>
   ...
   styles: [`
     .st-radio {
       ...
       .st-radio.disabled {
         opacity: 0.7; }
         .st-radio.disabled .custom-radio {
           cursor: default; }
       .st-radio .custom-radio {
         width: 15px;
         height: 15px;
         cursor: pointer;
         ...
         box-sizing: border-box;
         margin-right: 14px; }
         .st-radio .custom-radio.checked:before {
           display: block;
           position: absolute;
           content: '';
           border-radius: 100%;
